...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
		...
		<head>
			...
			<link href="${path}/css/viewbook.css" rel="stylesheet"> </head>
		...
		<body>
			...
			<div class="container">
				<h2 class="h2 page-header">View book</h2>
				    ...
				    <div class="row">
                    	<div class="col-md-3">
					        <p><label for="bookName">Name</label></p>
					        <p>${book.bookName}</p>
					    </div>
                        <div class="col-md-3">
                            <p><label for="bookRating">Rating</label></p>
                            <p>${book.bookRating}</p>
                        </div>
					    <div class="col-md-3">
                    ...
                    <div class="row">
                        <p><label for="publishers">Publisher</label></p>
							...
							<c:choose>
								<c:when test="${user.username == review.user.username}">
									...
									<c:forEach var="entry" items="${review.bookReview}">
										<p><textarea id="textReview" class="form-control" name="textReview" form="review">${entry.key}</textarea></p>

										<p><input type="number" name="rating" class="form-control" min="1" max="5" value="${entry.value}" form="review"/></p>
									</c:forEach>
									<p><button type="submit" class="btn btn-success" form="review">Edit review</button></p>
								</c:when>
								<c:otherwise>
									<c:forEach var="entry" items="${review.bookReview}">
									    <label for="username">Review by user: </label>
				...
				<form action="${path}/book/${book.bookId}/addreview" id="review" method="POST">
					...
					<c:if test="${empty rev}">
						<div id="but">
							<p><button type="button" class="btn btn-primary" onclick="addBookFormReview()">Add review</button></p>
					    </div>
					</c:if>
